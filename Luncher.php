<?php
 -define('BOT_TOKEN', '305167555:AAEfxI2SNKHnFQTenZfHzpRl_ZCM7J5Gh14');		 +/*
 -define('API_URL', 'https://api.telegram.org/bot'.BOT_TOKEN.'/');		 +سورس اصلی ربات پی وی رسان
 -		 +@PvResanBot
 -function apiRequestWebhook($method, $parameters) {		 +دست هیچ بنی بشری نیست و نخواهد بود
 -  if (!is_string($method)) {		 +
 -    error_log("Method name must be a string\n");		 +این سورس ای هم که داخل این گیت هاب هست
 -    return false;		 +سورس ورژن یک پی وی رسان هست
 -  }		 +و پی وی رسان هم اکنون از سورس ورژن های بالاتر استفاده میکند
 -		 +
 -  if (!$parameters) {		 +تیم برنامه نویسی FTC
 -    $parameters = array();		 +با آدرس سایت
 -  } else if (!is_array($parameters)) {		 +https://feelthecode.com
 -    error_log("Parameters must be an array\n");		 +
 -    return false;		 +اولین سازنده ربات پیام رسان فارسی در تلگرام
 -  }		 +و تنها برنامه نویس ربات پی وی رسان می باشد
 -		 +
 -  $parameters["method"] = $method;		 +لطفا از هیچکس هیچ سورس و یا آموزش تقلبی را خریداری نکنید
 -		 +نرم افزار های موجود در سطح اینترنت نیز
 -  header("Content-Type: application/json");		 +برای ساخت ربات پیام رسان
 -  echo json_encode($parameters);		 +کلاه برداری بیش نیستند
 -  return true;		 +
 -}		 +در صورت مشاهده این گونه افراد
 -		 +حتما با استفاده از فرم تماس با ما
 -function exec_curl_request($handle) {		 +موجود در وب سایت ما
 -  $response = curl_exec($handle);		 +یا از طریق تماس با کارشناسان ما
 -		 +ما را در جریان قرار دهید تا پیگیری های لازم
 -  if ($response === false) {		 +صورت پذیرد
 -    $errno = curl_errno($handle);		 +
 -    $error = curl_error($handle);		 +site: https://feelthecode.com
 -    error_log("Curl returned error $errno: $error\n");		 +Telegram Bot: https://telegram.me/PvResanBot
 -    curl_close($handle);		 +Telegram Channel: https://telegram.me/PvResan
 -    return false;		 +
 -  }		 +یا علی خدا قوت
 -		 +*/
